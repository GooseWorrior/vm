...
VM::VM(string filename)
    : state{0},
      ...
      theComponents{WindowSize, WindowPointer, vcursor, text, state, vmStatusLine, bufferCommand} {
  loadFile(filename);
}
...
void VM::process() {
  ...
  while (input != 'q') {
    ...
    prevSize = text.size();
    if (state == 1 || state == 2) prevCursor = pair<int, int>(vcursor.getRow(), vcursor.getCol()); 
