...
namespace CS246E {
    Cursor::Cursor(int row, int col, vector<string> & theText): theCursor{row, col}, theText{theText} {}
    Cursor & Cursor::operator++() {
        if (theCursor.second < theText[theCursor.first].size()) { theCursor.second++; }
        return *this;
    }
    Cursor & Cursor::operator--() {
        if (theCursor.second > 0) { theCursor.second--; }
        return *this;
    }
    Cursor & Cursor::nextLine() {
        if (theCursor.first == theText.size() - 1) { return *this; }
        else { theCursor.second = min<int>(theCursor.second, theText[++theCursor.first].size()); }
        return *this; 
    }
    Cursor & Cursor::prevLine() {
        if (theCursor.first == 0) { return *this; }
        else { theCursor.second = min<int>(theCursor.second, theText[--theCursor.first].size()); }
        return *this;
    }
    ...
    Cursor & Cursor::insert(wchar_t c) {
        if (c == '\n') {
            theText.insert(theText.begin() + theCursor.first + 1, theText[theCursor.first].substr(theCursor.second));
            theText[theCursor.first] = theText[theCursor.first].substr(0, theCursor.second);
            theCursor.second = 0; 
            theCursor.first++;
        } else {
