...
namespace CS246E {
   ...
   VMCommandLine{VMCommandLine}, VMStatusLine{VMCommandLine} {}
    ...
    pair<int, int> EditorComponent::getLocation(int type) {
        int row = winSize.first, col = 0; // little trick here: due to updateWindowSize in VM winSize.first == trueSize - 1
        switch (type) {
            case PERCENTAGE:
                col = winSize.second - 4; break;
            case MODE:
                col = 0; break;
            case LINENUMBER:
        ...
        switch (type) {
            case PERCENTAGE:
                if (winPtr.second == theText.size() - 1 && winPtr.first == 0) {
                    ...
                    ret = to_string((int)((100 * (vcursor.getRow() + 1)) / theText.size())) + "%";  
                }
                break;
            case MODE:
    ...
    void EditorComponent::print() {
        mvaddstr(winSize.first, 0, string(winSize.second, ' ').c_str()); // little trick here
        for (auto & i : components) {
            if (i.first == MODE) {
                attron(A_BOLD);
                i.second->print();
