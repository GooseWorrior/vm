...
const vector<string> keyWord{"if", "else", "while", "for", "do", "using", "try", "delete",
  "break", "case", "catch", "continue", "default", "dynamic_cast", "using", "static_cast",
  "friend", "goto", "new", "noexcept", "operator", "private", "const_cast","this",
  "protected", "public", "reinterpret_cast", "return", "throw", "typeid", "switch", "sizeof"};
  const vector<string> primitive{"auto", "bool", "char", "class", "const", "decltype", "double", "enum", "explicit", "extern",
  "float", "inline", "int", "long", "mutable", "namespace", "short", "signed", "static", "template", "typedef",
  "typename", "union", "unsigned", "virtual", "void"};
  const vector<string> special{"and", "or", "not", "true", "false", "nullptr", "NULL"};
  const vector<string> directiveType1{"#define", "#undef", "#if", "#elif", "#else", "#error", "#ifndef"};
  const vector<string> directiveType2{"#endif", "#line"};
  const vector<char> bracketList{'(', ')', '[', ']', '{', '}'};

SyntaxView::SyntaxView(VM * vm): View{}, vm{vm} {}

void SyntaxView::initialize() {
   loadLibrary();
}

void SyntaxView::loadLibrary() {
    for (auto i : special) {
      renderLibrary.push_back(pair<int, pair<regex, int>>(SPECIAL, pair<regex, int>(i, 6)));
    }
    for (auto i : keyWord) {
      renderLibrary.push_back(pair<int, pair<regex, int>>(KEYWORD, pair<regex, int>(i, 7)));
    } 
    for (auto i : primitive) {
      renderLibrary.push_back(pair<int, pair<regex, int>>(PRIMITIVE, pair<regex, int>(i, 4)));
    }
    for (auto i : directiveType1) {
      renderLibrary.push_back(pair<int, pair<regex, int>>(DELPHA, pair<regex, int>("^\\s*(" + i + ")($|\\s+)", 3)));
    }
    for (auto i : directiveType2) {
      renderLibrary.push_back(pair<int, pair<regex, int>>(DBETA, pair<regex, int>("^\\s*(" + i + ")($|\\s+)", 3)));
    }
    renderLibrary.push_back(pair<int, pair<regex, int>>(PREDIRECT, pair<regex, int>("^\\s*(#include)\\s*(<|\")", 3)));
    renderLibrary.push_back(pair<int, pair<regex, int>>(POSTDIRECT, pair<regex, int>("^\\s*#include\\s*(<.*>|\".*\")", 6)));
    renderLibrary.push_back(pair<int, pair<regex, int>>(STRING, pair<regex, int>("\"", 6)));
    renderLibrary.push_back(pair<int, pair<regex, int>>(NUMBER, pair<regex, int>("[\\d.]", 6)));
    renderLibrary.push_back(pair<int, pair<regex, int>>(SINGCOMMT, pair<regex, int>("\\/\\/", 1)));
    renderLibrary.push_back(pair<int, pair<regex, int>>(MULTCOMMTELPHA, pair<regex, int>("\\/\\*", 1)));
    renderLibrary.push_back(pair<int, pair<regex, int>>(MULTCOMMTBETA, pair<regex, int>("\\*\\/", 1)));
        ...
        for (auto it : renderLibrary) {
          ...
          while(regex_search(tempstr, m, it.second.first)) {
            if (m.size() > 0) {
                if (it.first == PREDIRECT) {
                  ...
                  mvchgat(loc1.first, loc1.second, 8, A_NORMAL, it.second.second, NULL);
                  ...
                  pair<int, int> loc = transferLoc(i, m.position(1));
                  mvchgat(loc.first, loc.second, -1, A_NORMAL, it.second.second, NULL);
                  break; 
                  ...
                  if (!std::isalpha(vm->text[i][m.position(0) + offset - 1]) && 
                      ...
                      (!std::isalpha(vm->text[i][m.position(0) + offset  + m.length(0)]) &&
                      vm->text[i][m.position(0) + offset + m.length(0)] != '_'))) { 
                        pair<int, int> loc = transferLoc(i, m.position(0) + offset);
                        mvchgat(loc.first, loc.second, m.length(0), A_NORMAL, it.second.second, NULL);
                  }
                } else {
                  if (!std::isalnum(vm->text[i][m.position(0) + offset - 1]) && 
                      vm->text[i][m.position(0) + offset - 1] != '_' &&
                      (m.position() == vm->text[i].size() || 
                      (!std::isalnum(vm->text[i][m.position(0) + offset  + m.length(0)]) &&
                      vm->text[i][m.position(0) + offset + m.length(0)] != '_'))) {
                        pair<int, int> loc = transferLoc(i, m.position(0) + offset);
                        mvchgat(loc.first, loc.second, m.length(0), A_NORMAL, it.second.second, NULL);
                  }
          ...
          for (auto it : quoteLoc) {
              if (frst == -1) {
                ...
                mvchgat(loc1.first, loc1.second, loc2.second - loc1.second + 1, A_NORMAL, 6, NULL);
                frst = -1;
              }
          }
          if (frst != -1) {
...
void SyntaxView::highlightBracket() {
  ...
  if (find(bracketList.begin(), bracketList.end(), vm->text[vm->vcursor.getRow()][vm->vcursor.getCol()]) != bracketList.end()) {
     local = pair<int, int>(vm->vcursor.getRow(), vm->vcursor.getCol());
  } else if (vm->vcursor.getCol() > 0 && find(bracketList.begin(), bracketList.end(), vm->text[vm->vcursor.getRow()][vm->vcursor.getCol() - 1]) != bracketList.end()) {
...
pair<int, int> SyntaxView::CloseBracket(char openBracket, char closeBracket, pair<int, int> local) {
  vector<bool> stack;
...
pair<int, int> SyntaxView::OpenBracket(char openBracket, char closeBracket, pair<int, int> local) {
  vector<bool> stack;
  for (int i = local.first; i >= 0; --i) {
    for (int j = local.first == i ? local.second - 1
                                      : vm->text[i].length() - 1;
         j >= 0; --j) {
...
void SyntaxView::printTextLine(int input, pair<int, int> prevCursor, int prevChar) {
